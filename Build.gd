...
func _check_place_object():
	if grid_map._get_grid_empty(object_to_place.translation.x, object_to_place.translation.z) == true:
...
func _place_object_allowed():
	print("----- GRID EMPTY -----")	
	grid_map._set_grid_contents(object_to_place.translation.x, object_to_place.translation.z, 1) #todo test value, change to obj id
	_reset_build()
